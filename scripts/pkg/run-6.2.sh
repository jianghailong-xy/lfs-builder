#!/usr/bin/env bash
# 宿主机侧驱动：在构建容器内以 lfs 用户执行 §6.2，完整输出落到日志。
set -uo pipefail
LFS_ROOT="${LFS_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
LOG=$LFS_ROOT/logs/packages/6.2-m4-1.4.21.log

{
  echo "##### LFS 13.0-systemd §6.2 M4-1.4.21"
  echo "##### 宿主机时间：$(date -Is)"
...
"$LFS_ROOT/scripts/lfs-container.sh" exec-lfs \
  'bash /workspace/scripts/pkg/6.2-m4.sh' >> "$LOG" 2>&1
rc=$?
