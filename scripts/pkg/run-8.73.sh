#!/usr/bin/env bash
set -uo pipefail
LFS_ROOT="${LFS_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"; LOG=$LFS_ROOT/logs/packages/8.73-tar-1.35.log; CONTAINER="${CONTAINER:-lfs-build-$(basename "$LFS_ROOT")}"; mkdir -p "$LFS_ROOT/logs/packages"
