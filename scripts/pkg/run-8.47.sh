...
LOG=$LFS_ROOT/logs/packages/8.47-autoconf-2.72.log
...

{
  ...
  echo "##### 官方手册：https://www.linuxfromscratch.org/lfs/view/13.0-systemd/chapter08/autoconf.html"
  echo "##### 执行前置验证、源码校验与解包、配置、编译、make check、安装、安装验证及清理"
  echo
  echo "===== chroot 环境准备 ====="
} >> "$LOG"
...
rc=$?
if [ "$rc" -eq 0 ]; then
  docker exec "$CONTAINER" bash /workspace/scripts/chroot.sh run /workspace/scripts/pkg/8.47-autoconf.sh >> "$LOG" 2>&1
  rc=$?
fi
echo "##### 最终退出码：$rc" >> "$LOG"
exit "$rc"
