#!/usr/bin/env bash
# LFS 13.0-systemd §6.12 Make-4.4.1
# 在构建容器内以 lfs 用户、手册 §4.4 的干净环境执行（由 lfs-container.sh exec-lfs 调用）。
#
# 手册 §6.12.1 的命令序列（全部，无补丁、无 sed、无测试套件）：
...
echo "===== LFS 13.0-systemd §6.12 Make-$VER ====="
echo "开始时间：$(date -Is)"
echo "手册数据：Approximate build time less than 0.1 SBU，Required disk space 15 MB"
echo
...
echo "可用空间（手册本节要求 15 MB）："
df -h "$LFS" | tail -n1
echo

echo "----- 前置检查：第 5 章交叉工具链 + §6.2~§6.11 产物必须可用 -----"
...
echo "上一任务（§6.11 Gzip-1.14）产物必须可用："
for f in usr/bin/gzip usr/bin/gunzip usr/bin/zcat; do
  [ -e "$LFS/$f" ] || { echo "错误：\$LFS/$f 缺失，§6.11 未完成" >&2; exit 1; }
...
echo "本节安装前 \$LFS/usr/bin 下不应已有 make（应为首次安装）："
for f in usr/bin/make; do
...
cd "$SRCDIR"
echo "源码目录：$PWD"
echo "Make 自报版本：$(grep -m1 -E "^PACKAGE_VERSION='" configure)"
echo "本节无补丁：手册 §6.12 未规定任何 patch（LFS 12.x 时代针对 make-4.4.1 的"
echo "  makeinfo/glibc-2.38 相关 sed 在 13.0 的 §6.12 中已不存在）。"
echo "本节也没有 sed 预处理：手册 §6.12 只有 configure/make/make install 三条命令。"
echo "注意：13.0 的 §6.12 configure 参数中已没有旧版本的 --without-guile，这里严格照抄 13.0 手册原文。"
echo

echo "================= 6.12.1 Installation of Make ================="
...
echo "================= 本节测试 ================="
echo "手册 §6.12 未规定任何测试：本节命令只有 ./configure、make、make DESTDIR=\$LFS install，"
echo "没有 make check（Make 的测试套件由第 8 章 §8.71 在 chroot 内执行："
echo "  chown -R tester . && su tester -c \"PATH=\$PATH make check\"）。"
...
echo "----- 安装结果检查（对照手册 §8.71.2 Contents of Make） -----"
rc=0
echo "1) 安装的程序：make（手册 §8.71.2 Installed program 仅列 make）"
for f in make; do
  p=$LFS/usr/bin/$f
  if [ -e "$p" ]; then
...
echo "2) 其他随包落盘的条目（信息性，记录实际安装了什么）："
for f in usr/bin/gmake usr/share/man/man1/make.1 usr/share/info/make.info \
         usr/include/gnumake.h usr/lib/gnumake.h; do
  if [ -e "$LFS/$f" ]; then printf '   INFO $LFS/%s 存在\n' "$f"
  else printf '   INFO $LFS/%s 未安装\n' "$f"; fi
done
echo "3) 权限与体积："
ls -l $LFS/usr/bin/make | sed "s|$LFS|\$LFS|" | sed 's/^/     /'
echo "4) 本次 make install 落盘的全部文件（DESTDIR=\$LFS，按修改时间挑出本次安装的）："
find $LFS/usr -newer $LFS/sources/$SRCDIR/config.log \( -type f -o -type l \) 2>/dev/null \
  | sed "s|$LFS|\$LFS|" | sort | sed 's/^/     /' || true
[ $rc -eq 0 ] || { echo "错误：Make 关键文件缺失或不符合手册要求" >&2; exit 1; }
echo

echo "----- 确认安装的是为目标平台交叉编译的 ELF（不是宿主二进制） -----"
echo "  make: $(file -b $LFS/usr/bin/make)"
echo "readelf 头（make）："
readelf -h $LFS/usr/bin/make | grep -E 'Class|Machine|Type' | sed 's/^/  /'
echo "动态依赖（应链接 §5.5 的 libc）："
$LFS_TGT-readelf -d $LFS/usr/bin/make | grep -E 'NEEDED' | sed 's/^/  /'
echo "解释器（应为 /lib64/ld-linux-x86-64.so.2）："
readelf -l $LFS/usr/bin/make | grep 'interpreter' | sed 's/^/  /'
echo "二进制中的版本字符串（应为 $VER）："
strings -a $LFS/usr/bin/make | grep -m2 -E "GNU Make|^$VER\$" | sed 's/^/  版本: /' || true
echo "（说明：本节产物只能在 chroot 之后运行，宿主上不执行 \$LFS/usr/bin/make --version）"
echo
