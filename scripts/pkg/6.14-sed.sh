#!/usr/bin/env bash
# LFS 13.0-systemd §6.14 Sed-4.9
# 在构建容器内以 lfs 用户、手册 §4.4 的干净环境执行（由 lfs-container.sh exec-lfs 调用）。
#
# 手册 §6.14.1 的命令序列（全部，无补丁、无 sed 改写、无测试套件）：
...
echo "===== LFS 13.0-systemd §6.14 Sed-$VER ====="
echo "开始时间：$(date -Is)"
echo "手册数据：Approximate build time 0.1 SBU，Required disk space 21 MB"
echo "手册简介：The Sed package contains a stream editor."
echo
...
echo "可用空间（手册本节要求 21 MB）："
df -h "$LFS" | tail -n1
echo

echo "----- 前置检查：第 5 章交叉工具链 + §6.2~§6.13 产物必须可用 -----"
...
echo "上一任务（§6.13 Patch-2.8）产物必须可用："
for f in usr/bin/patch; do
  [ -e "$LFS/$f" ] || { echo "错误：\$LFS/$f 缺失，§6.13 未完成" >&2; exit 1; }
...
echo "本节安装前 \$LFS/usr/bin 下不应已有 sed（应为首次安装）："
...
cd "$SRCDIR"
echo "源码目录：$PWD"
echo "Sed 自报版本：$(grep -m1 -E "^PACKAGE_VERSION='" configure)"
echo "本节无补丁：手册 §6.14 只有 ./configure、make、make DESTDIR=\$LFS install 三条命令，"
echo "  未规定任何 patch、sed 改写或额外的 configure 选项。"
echo

echo "================= 6.14.1 Installation of Sed ================="
...
echo "================= 本节测试 ================="
echo "手册 §6.14 未规定任何测试：本节命令只有 ./configure、make、make DESTDIR=\$LFS install，"
echo "没有 make check（Sed 的测试套件由第 8 章 §8.32 在 chroot 内执行：chown -R tester . / su tester -c \"PATH=\$PATH make check\"）。"
...
echo "----- 安装结果检查（对照手册 §8.32.2 Contents of Sed） -----"
rc=0
echo "1) 安装的程序：sed（手册 §8.32.2 Installed program 仅列 sed）"
for f in sed; do
  p=$LFS/usr/bin/$f
  if [ -e "$p" ]; then
...
for f in usr/share/man/man1/sed.1 usr/share/info/sed.info usr/share/doc/sed-$VER; do
  if [ -e "$LFS/$f" ]; then printf '   INFO $LFS/%s 存在\n' "$f"
  else printf '   INFO $LFS/%s 未安装\n' "$f"; fi
done
echo "3) 权限与体积："
ls -l $LFS/usr/bin/sed | sed "s|$LFS|\$LFS|" | sed 's/^/     /'
...
[ $rc -eq 0 ] || { echo "错误：Sed 关键文件缺失或不符合手册要求" >&2; exit 1; }
echo

echo "----- 确认安装的是为目标平台交叉编译的 ELF（不是宿主二进制） -----"
echo "  sed: $(file -b $LFS/usr/bin/sed)"
echo "readelf 头（sed）："
readelf -h $LFS/usr/bin/sed | grep -E 'Class|Machine|Type' | sed 's/^/  /'
echo "动态依赖（应链接 §5.5 的 libc）："
$LFS_TGT-readelf -d $LFS/usr/bin/sed | grep -E 'NEEDED' | sed 's/^/  /'
echo "解释器（应为 /lib64/ld-linux-x86-64.so.2）："
readelf -l $LFS/usr/bin/sed | grep 'interpreter' | sed 's/^/  /'
echo "二进制中的版本字符串（应为 $VER）："
strings -a $LFS/usr/bin/sed | grep -m2 -E "GNU sed|^$VER\$" | sed 's/^/  版本: /' || true
echo "（说明：本节产物只能在 chroot 之后运行，宿主上不执行 \$LFS/usr/bin/sed --version）"
echo
