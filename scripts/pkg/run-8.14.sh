...
# 在 chroot 内执行 §8.14 M4-1.4.21，完整输出落到
# logs/packages/8.14-m4-1.4.21.log。
set -uo pipefail
LFS_ROOT=/root/lfs
LOG=$LFS_ROOT/logs/packages/8.14-m4-1.4.21.log
...
{
  echo "##### chroot 准备（§8.14 前）—— 宿主机时间：$(date -Is)"
  echo
} >> "$PREP_LOG"
...
if [ $prep_rc -ne 0 ]; then
  echo "chroot 环境准备失败，按任务要求不继续 §8.14；保留日志 $PREP_LOG" >&2
  exit $prep_rc
fi

{
  echo "##### LFS 13.0-systemd §8.14 M4-1.4.21"
  echo "##### 宿主机时间：$(date -Is)"
  ...
  echo "#####   $PREP_LOG"
  echo "##### 上一任务 §8.13 Pcre2-10.47 已完成（日志 8.13-pcre2-10.47.log），其产物在"
  echo "#####   下方「前置检查」第 1 项中逐项确认。"
  echo "##### 手册原文快照：docs/book/chapter08-m4.html"
...
docker exec "$CONTAINER" \
  bash /workspace/scripts/chroot.sh run /workspace/scripts/pkg/8.14-m4.sh >> "$LOG" 2>&1
rc=$?

# chroot 内的脚本把 configure / make / make check / make install 的完整输出留在 /sources
# （= 宿主 sources/），这里把它们移进 logs/packages 作为留档，避免污染源码目录。
for pair in ".m4-configure.log:8.14-m4-1.4.21.configure.log" \
            ".m4-make.log:8.14-m4-1.4.21.make.log" \
            ".m4-make-check.log:8.14-m4-1.4.21.check.log" \
            ".m4-make-install.log:8.14-m4-1.4.21.install.log"; do
