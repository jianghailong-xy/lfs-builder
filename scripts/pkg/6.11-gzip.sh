#!/usr/bin/env bash
# LFS 13.0-systemd §6.11 Gzip-1.14
# 在构建容器内以 lfs 用户、手册 §4.4 的干净环境执行（由 lfs-container.sh exec-lfs 调用）。
#
# 手册 §6.11.1 的命令序列（全部，无补丁、无 sed、无测试套件）：
...
echo "===== LFS 13.0-systemd §6.11 Gzip-$VER ====="
echo "开始时间：$(date -Is)"
echo "手册数据：Approximate build time 0.1 SBU，Required disk space 12 MB"
echo
...
echo "可用空间（手册本节要求 12 MB）："
df -h "$LFS" | tail -n1
echo

echo "----- 前置检查：第 5 章交叉工具链 + §6.2~§6.10 产物必须可用 -----"
...
echo "上一任务（§6.10 Grep-3.12）产物必须可用："
for f in usr/bin/grep usr/bin/egrep usr/bin/fgrep; do
  [ -e "$LFS/$f" ] || { echo "错误：\$LFS/$f 缺失，§6.10 未完成" >&2; exit 1; }
...
echo "本节安装前 \$LFS/usr/bin 下不应已有 gzip（应为首次安装）："
for f in usr/bin/gzip usr/bin/gunzip usr/bin/zcat; do
...
cd "$SRCDIR"
echo "源码目录：$PWD"
echo "Gzip 自报版本：$(grep -m1 -E "^PACKAGE_VERSION='" configure)"
echo "本节无补丁：手册 §6.11 未规定任何 patch。"
echo "本节也没有 sed 预处理：手册 §8.67.1 的 sed -i 's/IO_ftrylockfile/IO_EOF_SEEN/' ... 之类"
echo "  在 13.0 的第 8 章 Gzip 中并不存在；本章 §6.11 只有 configure/make/make install 三条命令。"
echo

echo "================= 6.11.1 Installation of Gzip ================="
echo "----- configure（手册原文命令） -----"
echo "手册命令："
echo "  ./configure --prefix=/usr --host=\$LFS_TGT"
echo "（注意：与 §6.10 Grep 不同，手册在本节没有给 --build 参数，这里严格照抄手册。）"
...
echo "================= 本节测试 ================="
echo "手册 §6.11 未规定任何测试：本节命令只有 ./configure、make、make DESTDIR=\$LFS install，"
echo "没有 make check（Gzip 的测试套件由第 8 章 §8.67 在 chroot 内执行：那里才有 make check）。"
...
echo "----- 安装结果检查（对照手册 §8.67.2 Contents of Gzip） -----"
rc=0
echo "1) 安装的程序：gunzip, gzexe, gzip, uncompress, zcat, zcmp, zdiff,"
echo "   zegrep, zfgrep, zforce, zgrep, zless, zmore, znew"
for f in gunzip gzexe gzip uncompress zcat zcmp zdiff zegrep zfgrep zforce zgrep zless zmore znew; do
  p=$LFS/usr/bin/$f
  if [ -e "$p" ]; then
    printf '   OK   $LFS/usr/bin/%-11s %s\n' "$f" "$(file -b $p | cut -d, -f1-2)"
  else
    printf '   FAIL $LFS/usr/bin/%s 缺失\n' "$f"; rc=1
  fi
done
echo "2) uncompress 应为 gunzip 的硬链接（手册 §8.67.2 明确写 \"hard link with gunzip\"）："
ino_g=$(stat -c %i $LFS/usr/bin/gunzip 2>/dev/null || echo x)
ino_u=$(stat -c %i $LFS/usr/bin/uncompress 2>/dev/null || echo y)
if [ "$ino_g" = "$ino_u" ]; then
  printf '   OK   inode 相同（%s），确为硬链接，链接数 %s\n' "$ino_g" "$(stat -c %h $LFS/usr/bin/gunzip)"
else
  printf '   FAIL gunzip inode=%s，uncompress inode=%s，不是硬链接\n' "$ino_g" "$ino_u"; rc=1
fi
echo "3) 权限与体积："
ls -l $LFS/usr/bin/gzip $LFS/usr/bin/gunzip $LFS/usr/bin/uncompress $LFS/usr/bin/zcat \
  | sed "s|$LFS|\$LFS|" | sed 's/^/     /'
echo "4) 手册页抽查："
for f in gzip.1 gunzip.1 zcat.1 zdiff.1 zgrep.1 zless.1 zmore.1 znew.1 gzexe.1 zforce.1; do
  if [ -e "$LFS/usr/share/man/man1/$f" ]; then printf '   OK   $LFS/usr/share/man/man1/%s\n' "$f"
  else printf '   INFO $LFS/usr/share/man/man1/%s 未安装\n' "$f"; fi
done
echo "5) 信息性：本章不要求 info/doc，仅记录是否落盘"
for d in usr/share/info/gzip.info usr/share/doc/gzip; do
  if [ -e "$LFS/$d" ]; then printf '   INFO $LFS/%s 存在\n' "$d"
  else printf '   INFO $LFS/%s 未安装\n' "$d"; fi
done
[ $rc -eq 0 ] || { echo "错误：Gzip 关键文件缺失或不符合手册要求" >&2; exit 1; }
echo

echo "----- 确认安装的是为目标平台交叉编译的 ELF（不是宿主二进制） -----"
echo "  gzip: $(file -b $LFS/usr/bin/gzip)"
echo "readelf 头（gzip）："
readelf -h $LFS/usr/bin/gzip | grep -E 'Class|Machine|Type' | sed 's/^/  /'
echo "动态依赖（应链接 §5.5 的 libc）："
$LFS_TGT-readelf -d $LFS/usr/bin/gzip | grep -E 'NEEDED' | sed 's/^/  /'
echo "解释器（应为 /lib64/ld-linux-x86-64.so.2）："
readelf -l $LFS/usr/bin/gzip | grep 'interpreter' | sed 's/^/  /'
echo "二进制中的版本字符串（应为 $VER）："
strings -a $LFS/usr/bin/gzip | grep -m2 -E "gzip $VER|^$VER\$" | sed 's/^/  版本: /' || true
echo "shell 脚本类程序抽查（zcat/zgrep 是脚本，应引用 gzip）："
head -n1 $LFS/usr/bin/zcat | sed 's/^/  zcat 首行: /'
grep -m1 -o 'gzip' $LFS/usr/bin/zgrep | sed 's/^/  zgrep 引用: /' || true
echo "（说明：本节产物只能在 chroot 之后运行，宿主上不执行 \$LFS/usr/bin/gzip --version）"
echo
