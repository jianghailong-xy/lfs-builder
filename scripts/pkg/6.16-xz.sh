#!/usr/bin/env bash
# LFS 13.0-systemd §6.16 Xz-5.8.2
# 在构建容器内以 lfs 用户、手册 §4.4 的干净环境执行（由 lfs-container.sh exec-lfs 调用）。
#
# 手册 §6.16.1 的命令序列（全部，无补丁、无 sed 改写、无测试套件）：
...
echo "===== LFS 13.0-systemd §6.16 Xz-$VER ====="
echo "开始时间：$(date -Is)"
...
echo "----- 前置检查：第 5 章交叉工具链 + §6.2~§6.15 产物必须可用 -----"
...
echo "上一任务（§6.15 Tar-1.35）产物必须可用："
for f in usr/bin/tar; do
  [ -e "$LFS/$f" ] || { echo "错误：\$LFS/$f 缺失，§6.15 未完成" >&2; exit 1; }
...
echo "本节安装前 \$LFS/usr/bin 下不应已有 xz（应为首次安装）："
for f in usr/bin/xz usr/lib/liblzma.so; do
...
cd "$SRCDIR"
echo "源码目录：$PWD"
echo "Xz 自报版本：$(grep -m1 -E "^PACKAGE_VERSION='" configure)"
echo "本节无补丁：手册 §6.16 只有 ./configure、make、make DESTDIR=\$LFS install、"
echo "  rm -v \$LFS/usr/lib/liblzma.la 四条命令，未规定任何 patch 或 sed 改写。"
echo

echo "================= 6.16.1 Installation of Xz ================="
...
echo "build-aux/config.guess 的输出：$(build-aux/config.guess)"
...
grep -E '^(prefix|docdir|CC) *=' Makefile | head -n5 | sed 's/^/  /' || true
echo "--disable-static 生效确认（应 enable_static=no / build_old_libs=no）："
grep -m1 -E "^enable_static=" config.log | sed 's/^/  /' || true
grep -m1 -E "^build_old_libs=" libtool 2>/dev/null | sed 's/^/  /' || true
...
echo "----- 手册要求：删除对交叉编译有害的 libtool 归档文件 -----"
echo "手册命令：rm -v \$LFS/usr/lib/liblzma.la"
rm -v $LFS/usr/lib/liblzma.la
[ -e "$LFS/usr/lib/liblzma.la" ] && { echo "错误：liblzma.la 仍存在" >&2; exit 1; }
echo "确认 \$LFS/usr/lib 下已无 liblzma.la："
find $LFS/usr/lib -maxdepth 1 -name '*.la' | sed "s|$LFS|\$LFS|" | sed 's/^/  /' || true
echo "  （上面若无输出即为无残留 .la）"
echo

echo "================= 本节测试 ================="
echo "手册 §6.16 未规定任何测试：本节命令只有 ./configure、make、make DESTDIR=\$LFS install"
echo "和 rm -v \$LFS/usr/lib/liblzma.la，没有 make check（Xz 的测试套件由第 8 章 §8.8 在"
echo "chroot 内执行：make check）。"
...
echo "----- 安装结果检查（对照手册 §8.8.2 Contents of Xz） -----"
rc=0
echo "1) 安装的程序（手册 §8.8.2 Installed programs）："
for f in lzcat lzcmp lzdiff lzegrep lzfgrep lzgrep lzless lzma lzmadec lzmainfo \
         lzmore unlzma unxz xz xzcat xzcmp xzdec xzdiff xzegrep xzfgrep xzgrep \
         xzless xzmore; do
  p=$LFS/usr/bin/$f
  if [ -e "$p" ]; then
    printf '   OK   $LFS/usr/bin/%-9s %s\n' "$f" "$(file -b $p | cut -d, -f1-2)"
  else
    printf '   FAIL $LFS/usr/bin/%s 缺失\n' "$f"; rc=1
  fi
done
echo "2) 安装的库（手册 §8.8.2 Installed library：liblzma.so）："
for f in usr/lib/liblzma.so usr/lib/liblzma.so.5; do
  if [ -e "$LFS/$f" ]; then printf '   OK   $LFS/%-22s %s\n' "$f" "$(file -b $LFS/$f | cut -d, -f1-2)"
  else printf '   FAIL $LFS/%s 缺失\n' "$f"; rc=1; fi
done
echo "   静态库不应存在（--disable-static）："
if [ -e "$LFS/usr/lib/liblzma.a" ]; then printf '   FAIL $LFS/usr/lib/liblzma.a 存在，--disable-static 未生效\n'; rc=1
else printf '   OK   $LFS/usr/lib/liblzma.a 不存在\n'; fi
echo "3) 安装的目录（手册 §8.8.2 Installed directories）："
for d in usr/include/lzma usr/share/doc/xz-$VER; do
  if [ -d "$LFS/$d" ]; then printf '   OK   $LFS/%-26s（%s 项）\n' "$d" "$(ls -A $LFS/$d | wc -l)"
  else printf '   FAIL $LFS/%s 缺失\n' "$d"; rc=1; fi
done
echo "4) 权限与体积："
ls -l $LFS/usr/bin/xz $LFS/usr/lib/liblzma.so* | sed "s|$LFS|\$LFS|" | sed 's/^/     /'
echo "5) 本次 make install 落盘的全部文件（DESTDIR=\$LFS，按修改时间挑出本次安装的）："
find $LFS/usr -newer $LFS/sources/$SRCDIR/config.log \( -type f -o -type l \) 2>/dev/null \
  | sed "s|$LFS|\$LFS|" | sort | sed 's/^/     /' || true
[ $rc -eq 0 ] || { echo "错误：Xz 关键文件缺失或不符合手册要求" >&2; exit 1; }
echo

echo "----- 确认安装的是为目标平台交叉编译的 ELF（不是宿主二进制） -----"
echo "  xz:          $(file -b $LFS/usr/bin/xz)"
echo "  liblzma.so:  $(file -b $LFS/usr/lib/liblzma.so)"
echo "readelf 头（xz）："
readelf -h $LFS/usr/bin/xz | grep -E 'Class|Machine|Type' | sed 's/^/  /'
echo "动态依赖（应链接 §5.5 的 libc 与本包的 liblzma）："
$LFS_TGT-readelf -d $LFS/usr/bin/xz | grep -E 'NEEDED' | sed 's/^/  /'
echo "解释器（应为 /lib64/ld-linux-x86-64.so.2）："
readelf -l $LFS/usr/bin/xz | grep 'interpreter' | sed 's/^/  /'
echo "二进制中的版本字符串（应为 $VER）："
strings -a $LFS/usr/bin/xz | grep -m2 -E "^$VER$|xz \(XZ Utils\)" | sed 's/^/  版本: /' || true
echo "脚本类文件确认（xzgrep 等为 POSIX shell 脚本，非 ELF）："
file -b $LFS/usr/bin/xzgrep | sed 's/^/  xzgrep: /'
echo "（说明：本节产物只能在 chroot 之后运行，宿主上不执行 \$LFS/usr/bin/xz --version）"
echo
