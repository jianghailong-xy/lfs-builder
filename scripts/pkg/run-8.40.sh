...
LOG=$LFS_ROOT/logs/packages/8.40-gperf-3.3.log
...

{
  ...
  echo "##### 官方手册：https://www.linuxfromscratch.org/lfs/view/13.0-systemd/chapter08/gperf.html"
  echo "##### 本节无补丁命令；执行 configure、make、make check、make install、验证及清理"
  echo
  echo "===== chroot 环境准备 ====="
} >> "$LOG"
...
rc=$?
if [ "$rc" -eq 0 ]; then
  docker exec "$CONTAINER" bash /workspace/scripts/chroot.sh run /workspace/scripts/pkg/8.40-gperf.sh >> "$LOG" 2>&1
  rc=$?
fi
echo "##### 最终退出码：$rc" >> "$LOG"
exit "$rc"
