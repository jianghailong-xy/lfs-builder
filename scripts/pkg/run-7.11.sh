...
if [ $prep_rc -ne 0 ]; then
  echo "chroot 环境准备失败，按任务要求不继续 §7.11；保留日志 $PREP_LOG" >&2
  exit $prep_rc
fi

{
  echo "##### LFS 13.0-systemd §7.11 Texinfo-7.2（临时工具）"
  echo "##### 宿主机时间：$(date -Is)"
...
docker exec "$CONTAINER" \
  bash /workspace/scripts/chroot.sh run /workspace/scripts/pkg/7.11-texinfo.sh >> "$LOG" 2>&1
rc=$?
