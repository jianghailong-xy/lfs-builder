...
set -uo pipefail
LFS_ROOT=/root/lfs
LOG=$LFS_ROOT/logs/packages/8.76-markupsafe-3.0.3.log
...

{
  ...
  echo "##### 执行前置验证、源码校验与解包、手册规定编译/测试说明/安装、验证及清理"
  echo
  echo "===== chroot 环境准备 ====="
} >> "$LOG"
...
rc=$?
if [ "$rc" -eq 0 ]; then
  docker exec "$CONTAINER" bash /workspace/scripts/chroot.sh run /workspace/scripts/pkg/8.76-markupsafe.sh >> "$LOG" 2>&1
  rc=$?
fi
echo "##### 最终退出码：$rc" >> "$LOG"
exit "$rc"
