...
echo "手册存档：/workspace/docs/book/chapter08-file.html（宿主机 /root/lfs/docs/book/）"
echo
...
echo "  configure  完整输出：$CONFLOG（= 宿主 /root/lfs/sources/.file-configure.log）"
echo "  make check 完整输出：$CHECKLOG（= 宿主 /root/lfs/sources/.file-make-check.log）"
cd /sources
rm -rf "$SRCDIR"
