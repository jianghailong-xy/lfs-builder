...
# 在 chroot 内执行 §7.12 Util-linux-2.41.3，完整输出落到 logs/packages/7.12-util-linux-2.41.3.log。
set -uo pipefail
LFS_ROOT=/root/lfs
LOG=$LFS_ROOT/logs/packages/7.12-util-linux-2.41.3.log
...
if [ $prep_rc -ne 0 ]; then
  echo "chroot 环境准备失败，按任务要求不继续 §7.12；保留日志 $PREP_LOG" >&2
  exit $prep_rc
fi

{
  echo "##### LFS 13.0-systemd §7.12 Util-linux-2.41.3（临时工具）"
  echo "##### 宿主机时间：$(date -Is)"
...
docker exec "$CONTAINER" \
  bash /workspace/scripts/chroot.sh run /workspace/scripts/pkg/7.12-util-linux.sh >> "$LOG" 2>&1
rc=$?
