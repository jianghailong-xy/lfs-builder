...
LOG=$LFS_ROOT/logs/packages/8.60-kmod-34.2.log
...

{
  ...
  echo "##### 官方手册：https://www.linuxfromscratch.org/lfs/view/13.0-systemd/chapter08/kmod.html"
  echo "##### 执行前置验证、源码校验与解包、配置、编译、测试、安装、验证及清理"
  echo
  echo "===== chroot 环境准备 ====="
} >> "$LOG"
...
rc=$?
if [ "$rc" -eq 0 ]; then
  docker exec "$CONTAINER" bash /workspace/scripts/chroot.sh run /workspace/scripts/pkg/8.60-kmod.sh >> "$LOG" 2>&1
  rc=$?
fi
echo "##### 最终退出码：$rc" >> "$LOG"
exit "$rc"
